...
- **Inside the container**, ghost applies a **per-exec process sandbox** to each command it spawns — filesystem restriction (Landlock) and process limits, plus an *attempt* at network-namespace isolation. The first two hold under any posture; the netns isolation does **not** hold under the hardened posture this RFD mandates (see below), so containing the student command's network is the *container* layer's job, not the per-exec sandbox's. This is the boundary around the *student's code* each time it runs.
